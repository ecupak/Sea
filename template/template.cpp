...
void main()
{
...
#else
#if STRETCHWINDOW > 0
	window = glfwCreateWindow( SCRWIDTH * STRETCHWINDOW, SCRHEIGHT * STRETCHWINDOW, "DoubleSize", 0, 0 );
#else
	...
	{
		deltaTime = min( 0.5f, timer.elapsed() );
		timer.reset();
		app->Tick( deltaTime );
	...
	// Terminate app.
	app->Shutdown();
	//delete app; // miniaudio crash if line runs.
	...
	// Terminate GLFW. Must delete shader first.
	delete shader;
	delete renderTarget;
...
// [Credit] Bogdan (230005)
void InitRandomSeedThread::Main()
{
	// Convert thread id into a random seed.
	std::thread::id threadId = std::this_thread::get_id();
	const size_t id = std::hash<std::thread::id>{}(threadId);

	InitSeedFromThread(static_cast<uint>(id));
}
