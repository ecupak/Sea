...
// Audio (mini-audio wrapper by Jeremiah).
// [Credit] https://github.com/jpvanoosten/Audio
...
inline uint float3_to_uint(const float3& f)
{
	uint r{ static_cast<uint>(255.0f * min(1.0f, f.x)) };
	uint g{ static_cast<uint>(255.0f * min(1.0f, f.y)) };
	uint b{ static_cast<uint>(255.0f * min(1.0f, f.z)) };
